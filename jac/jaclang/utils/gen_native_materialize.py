...
def generate(LAYOUT: dict, OUT_JAC: Path) -> dict:
    """Generate the materializer root at OUT_JAC from a unitree-closure
    layout dict; returns the runtime manifest (class names, key names, enum
    members) the seal embeds into the artifact layout."""
    ...
    # no emitted attr read may collide with a seal demotion waiver: emitters
    # touch masked has-fields only, and this pins that invariant at
    # generation time so the artifact can never reach an abort stub
    from jaclang.utils.precompile_bytecode import _UNITREE_SEAM_WAIVERS
    waived_attrs = {
        w.split("::")[-1].split(".")[1] for w in _UNITREE_SEAM_WAIVERS
    }
    emitted_reads = set(re.findall(r"nd\.(\w+)", "\n\n".join(emit_fns)))
...
if __name__ == "__main__":
    manifest = generate(
        ...
        Path(sys.argv[2]) if len(sys.argv) > 2 else Path("native_materialize.jac"),
    )
    print(
        f"classes {len(manifest['class_names'])}, keys "
        f"{len(manifest['key_names'])}, enums {len(manifest['enums'])}"
    )
