...
**GraphMend.** GraphMend (PyTorch 2 FX graph-break elimination) is on by default in `auto` mode: it engages per compiled module, exactly for modules that import `torch` or a `torch.*` submodule; every other module compiles byte-identically to a graphmend-off run. Within an engaged module, only functions reachable from an entry point are transformed. An entry point is a `torch.compile` decorated function or class, a call-site wrap such as `torch.compile(Model())`, or a `forward` method whose class binds a torch Module base; functions not reachable from any entry are left untouched, and a module with no reachable entries compiles exactly as off, sharing the off-mode cache artifact. Imported plain `.py` model code is claimed automatically: a module that holds an entry point claims itself, and claiming follows its eager imports within the same top-level package, so a library module reached only through its importer is transformed too. Imports guarded by `if TYPE_CHECKING:` never extend the claim, and `torch`, `jaclang` and the standard library are never claimed. The mode is set in `jac.toml` (`[run] graphmend = "auto" | "on" | "off"`), not by a CLI flag; `--graphmend-disable` has a matching `[run] graphmend_disable` key, with the CLI beating `jac.toml`. GraphMend-active compiles stay cached, keyed by the GraphMend configuration and by the claim state that decides what the module's transform can see.
