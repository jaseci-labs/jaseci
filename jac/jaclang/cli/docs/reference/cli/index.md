...
**GraphMend.** GraphMend (PyTorch 2 FX graph-break elimination) is on by default in `auto` mode: it engages per compiled module, exactly for modules that import `torch` or a `torch.*` submodule; every other module compiles byte-identically to a graphmend-off run. Within an engaged module, only functions reachable from a `torch.compile` entry point (decorated, call-site wrapped, or scoped) are transformed; functions not reachable from any entry are left untouched, and a module with no reachable entries compiles exactly as off, sharing the off-mode cache artifact. The mode is set in `jac.toml` (`[run] graphmend = "auto" | "on" | "off"`), not by a CLI flag. The two flags above have matching `[run]` keys (`graphmend_scope`, `graphmend_disable`), with the CLI beating `jac.toml`. GraphMend-active compiles stay cached, keyed by the GraphMend configuration.
