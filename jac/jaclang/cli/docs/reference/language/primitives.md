...
- **Server is the default.** Python imports, graph archetypes, and similar constructs anchor a module server, and unreferenced pure code compiles to the Python backend, exactly as before.
...
| Codespace | How code gets there | Variant suffix | Compiles To | Ecosystem |
|-----------|--------------------|----------------|-------------|-----------|
| Server    | inferred (the default; python imports and graph archetypes anchor); `[placement.pins]` -> `"server"` | -- | Python | PyPI |
| Client    | inferred (JSX, browser globals, npm imports); `[placement.pins]` -> `"client"` | `.cl.jac` (implementation variant) | JavaScript / TypeScript | npm |
| Native    | inferred (extern C declarations); `[placement.pins]` -> `"native"` | `.na.jac` (implementation variant) | LLVM IR → machine code | C ABI |
