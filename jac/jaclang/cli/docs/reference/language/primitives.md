...
- **Pins always win.** Inference never overrides a `[placement.pins]` entry (or a `.cl.jac` implementation variant / `variant` declaration).
...
| Native    | inferred (extern C declarations, whole-module verdict); `[placement.pins]` -> `"native"` | `variant native;` declaration (no filename suffix) | LLVM IR → machine code | C ABI |
