...
- **Pins always win.** Inference never overrides a `[placement.pins]` entry (or a `.jac` implementation variant).
...
| Client    | inferred (JSX, browser globals, npm imports); `[placement.pins]` -> `"client"` | `.jac` (implementation variant) | JavaScript / TypeScript | npm |
| Native    | inferred (extern C declarations, whole-module verdict); `[placement.pins]` -> `"native"` | -- (no per-file spelling; forced via `jac nacompile`) | LLVM IR → machine code | C ABI |
