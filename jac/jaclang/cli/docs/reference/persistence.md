...
- **The cluster is not in `$HOME`.** `JAC_DATA_DIR` decides where the cluster lives: `<JAC_DATA_DIR>/pg/main` (default `~/.local/share/jac`). The image pins it to a shared directory so the path does not change with the uid the pod runs as; that is also the one path to mount if you want the cluster to survive a restart.
