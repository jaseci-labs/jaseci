...
- The passes run in `get_py_code_gen`, immediately before `PyastGenPass`:
  ...
  `PredicateCtrlFlowPass`, then `DeferSideEffectPass`. The order is
  load-bearing: trap lowering removes
...
happens to match a logging pattern is never rewritten.
...
`test_graphmend_defer_integration.jac` (skipped without torch) does the same
for deferral: print and logger fixtures defragment to one graph, a buffered
