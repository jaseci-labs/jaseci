...
| `E1311` | Every `freeze(x)` argument is statically unique at the call: an `own` binding (consumed by the call) or a fresh expression. Uniqueness is the proof that no other handle can ever write the frozen value, which is what lets `freeze` be the identity function at runtime on every backend. |
