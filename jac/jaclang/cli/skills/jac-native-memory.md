...
- `freeze(x)` promotes a statically unique value (an `own` binding, consumed, or a fresh expression) into the `imm` world; identity at runtime on every backend, E1311 when uniqueness cannot be proven. Frozen values are the natural `flow` payload (imm crosses freely).
