...
- `flow for x in &xs { }` / `flow for m in &mut xs { }` - the disjoint-partition loop: per-element body, closing brace is the join. Collection must be lent, `break`/`return`/`yield`/`disengage` in the body are E1313 (`continue` ok), nesting rejected, and body captures follow sendability (write to any outer name is E1308 - write through the `&mut` element instead). Currently lowers sequentially on both backends (same as native `flow` calls); the checked semantics are what parallel execution will honor.
