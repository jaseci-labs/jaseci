...
- **An `app` that ignores `children`** - e.g. the single-page shape `def:pub app -> JsxElement { <Home/> }` - **drops every route**, so `pages/` never renders and the app stays stuck on one page. In a project with a `pages/` directory this is now `E1108` at `jac check` time rather than a silent blank screen. A single `props` parameter satisfies it too, since the route tree arrives as `props.children`.
