...
- **Top-level component name is `def:pub app`** - lowercase. Runtime mounts the literal name. Its signature is set by the routing system: `app()` for manual/single-page, `app(children: any)` that renders `children` for file-based routing. Getting this wrong drops every route; in a `pages/` project it is caught as `E1108` - see `jac-cl-routing`.
