...
- **Consuming a native module is NOT a signal.** `import from mymod { fast_fn }` where `mymod` is native code stays a server-side import - that is the server-to-native ctypes interop crossing, not a reason to relocate the importer. The client-side crossing is spelled explicitly: `na import from .mymod { fast_fn }` in a client module compiles the target to `/static/<stem>.wasm`, binds the names to lazy async wasm stubs, and compiles to nothing on the server (see `jac-native-wasm`). A *plain* client import takes the same edge only when the target is decidedly native - already native-compiled, or carrying a real native anchor (ownership annotations, clib extern decls, native imports); a plain `pub` module with no anchor reads as a server endpoint and bridges over RPC instead.
