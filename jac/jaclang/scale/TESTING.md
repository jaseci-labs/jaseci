...
- **End to end** carries the most weight: a real `jac start`, a real deploy to
...
`jac start` against a fixture app; for local fleet changes it means an app
with a `[scale.microservices.routes]` table so the gateway and services really
come up.
...
- **The dev-source override.** Covered in 1.2, listed again because it is the
  ...
  edits while `jac start` from the app directory runs baked jaclang.
