...
The binary is built with [Zig](https://ziglang.org/) **0.16.0** (the version is pinned -- newer/older majors will fail to build). Zig plus a network connection are the only build-time deps: the payload tool (`jaclang.payload`, Jac on the bundled Python tier that `zig build` boots) does all the HTTP fetching, integrity checks, and compression, so there's nothing else to install (the old `curl`/`git`/`zstd`/`tar` shellouts are gone).
