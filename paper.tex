...
$1.13\times$ ratio is a small fixed cost seen at one small size that amortizes to
...
returned elements, resolves the crossing into a $\approx$$15$\,ms fixed cost plus
an $890$\,ns/element serialization slope (95\% bootstrap CI $848$--$983$)---$2.6\times$ the in-process list's
$342$\,ns/element---with byte-identical digests at every size, and directly
observes that slope overtake the fixed cost at $N\approx17{,}000$. We further report
...
eight-point sweep of Section~\ref{sec:sweep}), all three family-2 cross-runtime
cells, and the payload sweep (Section~\ref{sec:payload}) are all captured under
...
construction, with no fixed floor to speak of. The \code{rpc} path is the mirror
...
$848$--$983$) on a $\approx$$15$\,ms floor ($R^2=0.994$); the earlier sweep, which
stopped at $N{=}20{,}000$---essentially at break-even (below)---could only
extrapolate this slope, but the extended range now measures it directly. That is
...
floor (CV $7.2\%$, $14.6$--$17.0$\,ms), which is exactly the loopback/HTTP setup
cost and does not touch the slope. Coefficients are also reported with a within-run
...
$N\approx17{,}000$ elements---almost exactly where the earlier sweep stopped, which
is why it could not see past the floor---and the extended sweep measures $N$ nearly
six times beyond it: at $N{=}100{,}000$ the slope term ($\approx$$89$\,ms) is
$\approx$$5.9\times$ the $\approx$$15$\,ms fixed cost, so the crossing is firmly
\emph{slope-dominated}. This inverts the earlier reading---bounded at $20{,}000$,
the crossing looked fixed-cost-dominated throughout and the slope was a
second-order extrapolation---and replaces it: the loopback RPC is a fixed-cost
...
($\approx$$42\times$, \code{na} vs.\ \code{sv})---so the $1.13\times$ callback
...
$\approx$$15$\,ms fixed cost plus an $890$\,ns/element serialization slope
(95\% bootstrap CI $848$--$983$, run-to-run CV $1.8\%$),
$2.6\times$ its in-process twin and directly observed to overtake the fixed cost
at $N\approx17{,}000$. We add low-nanosecond \code{C}-ABI foreign-call
