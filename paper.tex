...
break-even at $N\approx17{,}000$. A committed, oracle-gated cross-tool matrix
...
costs $\approx$$10\times$ a hand-written \code{FastAPI} client on the boundary
...
\emph{signature-dependent}, not one number. Isolated per-call cost stays in a
tight $23$--$61$\,ns band for the scalar across the compiled bindings, but a
\emph{by-value struct} widens it by an order of magnitude on the descriptor-driven
tools---\code{ctypes} and \code{cffi} pay $\approx$$1.1$\,\textmu s marshalling
the copy, while \code{METH\_O}, \code{pybind11}, and \code{PyO3} stay at
$87$--$134$\,ns---exactly the register-vs-\code{byval} ABI distinction a compiler
...
between the two ($65$--$145$\,ns compiled, $374$--$590$\,ns descriptor-driven).
The claim ``Jac-native FFI beats the best CPython bindings by
$\approx$$10\times$ on mechanism'' therefore holds not just for a scalar but
across three signature shapes.
...
\code{work}${=}1$, callee arithmetic differenced out) Jac's generated crossing is
$\approx$$9.7\times$ the hand-written \code{FastAPI} client and
$\approx$$25\times$ the minimal endpoint---the same
...
no measurable cost over hand \code{httpx} ($\approx$$1.5$ vs.\ $1.6$\,ms), so the
gap is the app-server$+$\code{sv~import} stack, not client-side typing---%
corroborating the $\approx$$5$\,ms framework term above from an independent,
matched comparand rather than by subtraction. The harness also records a TCP
connect \code{RTT} per comparand: $\approx$$30$\,\textmu s on loopback, i.e.\ the
$15$\,ms boundary is $\approx$$500\times$ the wire, confirming across the whole
