...
\emph{slope} ($5$ vs.\ $212$\,ns/iter, \code{na} vs.\ \code{sv}), while the
\code{native$\rightarrow$server} callback crossing is a fixed
$\approx$$2.0$\,\textmu s per-call \emph{intercept}---so its headline
$1.17\times$ ratio is a small fixed cost seen at one small size that amortizes to
$\approx$$1.0\times$ once the callee does real work. Family-1 timings are captured
under a pinned CPU governor with turbo disabled; the ratios are reproducible
while the absolute nanosecond figures are clock-specific. A complementary payload sweep over the same loopback RPC, from $1$ to $100{,}000$
returned elements, resolves the crossing into a $\approx$$17$\,ms fixed cost plus
a $911$\,ns/element serialization slope (95\% bootstrap CI $889$--$1002$)---$2.7\times$ the in-process list's
$342$\,ns/element---with byte-identical digests at every size, and directly
observes that slope overtake the fixed cost at $N\approx19{,}000$. We further report
low-nanosecond \code{C}-ABI foreign-call costs and, for the two loopback RPC
cells re-captured under the pinned governor (median of three runs at $n{=}20$,
run-to-run CV $\le$$4\%$), cross-runtime floors of $241\times$ (\code{xop\_feed})
and $374\times$ (\code{xop\_svc\_split}) over the direct dispatch they replace. The
...
frequency non-determinism, not to the toolchain. First, the same \code{sv}-resident
...
eight-point sweep of Section~\ref{sec:sweep}) and all three family-2
cross-runtime cells are captured under the pinned governor with turbo disabled
(Section~\ref{sec:controls}), as medians of three runs at $n{=}20$; the payload
sweep (Section~\ref{sec:payload}) is a single pinned sweep at $n{=}20$ per size.
Every cell's variants produced byte-identical canonical digests---the oracle
passed for all ten measured kernels at every size, independent of clock.
...
so we read this as a $\approx$$13\%$ effect, not a hairline one. This is the
suite's cleanest statement of pure boundary overhead---and it is smaller than the
$1.17\times$ reported from the earlier uncontrolled capture, consistent with the
callback fixed cost shrinking to $\approx$$2$\,\textmu s under pinned clock
...
re-captured under the pinned governor and disabled turbo of
Section~\ref{sec:controls} as medians of three runs at $n{=}20$ interleaved
invocations. For the two loopback RPC cells (\code{xop\_svc\_split},
...
$<$$4\%$, native CV $<$$0.1\%$). The marshalled crossings are up to two-plus
orders of magnitude more expensive than the direct dispatch they replace, as
physics demands: a loopback HTTP+JSON RPC (\code{xop\_svc\_split}) costs
$374\times$ its in-process twin and a generated-client feed (\code{xop\_feed})
$241\times$ its direct provider call, while a \code{wasm32} export
(\code{xop\_wasm\_call}) is $6.8\times$ a native bridge under an asymmetric host
loop. The RPC crossing-side spread is wide (the RPC medians vary $189$--$309$\,M\,ns
...
(An earlier uncontrolled-clock capture reported this gap as $\approx$$5.4$\,\textmu s;
the twin-gap \emph{method} is unchanged but the magnitude was inflated by
CPU-frequency drift, and $\approx$$2.0$\,\textmu s is the pinned-clock value; see
Section~\ref{sec:controls}.) That fixed cost is why the \code{iop\_cb} crossing
...
is dominated by timer noise rather than the boundary. The much-quoted
``$1.17\times$'' is not a property of the boundary; it is a small fixed cost seen
at one small size that vanishes once the callee does real work. \code{iop\_symmetric}
...
noise-dominated (they scatter $0.83$--$1.24$ rather than tracing a clean decay);
the fixed cost is established by the twin-gap fit, not by these raw ratios. An
earlier uncontrolled-clock capture showed an apparent smooth decay from
$1.73\times$; that monotonicity did not survive frequency pinning.}
\label{fig:decay}
\end{figure}
...
(95\% CI $340$--$343$, $R^2>0.9999$ over the whole range): building an
...
$\approx$$17$\,ms fixed per-call cost dominates at small $N$, and the per-element
serialization slope is buried beneath it until the payload grows large. Over the
full range the \code{rpc} fit is $s=911$\,ns/element (95\% CI $889$--$1002$) on a
$\approx$$17$\,ms floor ($R^2=0.997$); the earlier sweep, which stopped at
...
to the post-crossing band ($N\ge30{,}000$) gives $s=823$\,ns/element
(95\% CI $808$--$846$) on a $\approx$$23$\,ms floor at $R^2=0.9998$. That is the
result: \emph{crossing the RPC boundary costs $\approx$$2.7\times$ per element
what the in-process list costs} ($911$ vs.\ $342$\,ns/element)---the
...
collapses from $\approx$$21{,}700\times$ at $N{=}1$ to $\approx$$3\times$ at
...
Crucially, the crossing is now \emph{observed}, not inferred, and the fit
coefficients are reported with bootstrap confidence intervals rather than bare
spread (percentile pairs bootstrap over the sixteen sweep points, $B{=}10^4$).
The break-even payload at which the serialization term equals the fixed cost is
$N\approx19{,}000$ elements---almost exactly where the earlier sweep stopped,
which is why it could not see past the floor---and the extended sweep measures
$N$ more than five times beyond it: at $N{=}100{,}000$ the slope term
($\approx$$91$\,ms) is $\approx$$5.4\times$ the $\approx$$17$\,ms fixed cost, so
the crossing is firmly \emph{slope-dominated}. This inverts the earlier
...
``order-of-magnitude'': the full-range slope is $911$\,ns/element (CI
$889$--$1002$), tightening to $808$--$846$ in the clean post-crossing regime,
while the per-point crossing medians stay wide at small $N$. That small-$N$ width
is the fixed-cost floor's noise, not the slope's: the \code{rpc} medians are
non-monotone only for $N\le2000$ and become strictly monotone from $N{=}5000$
...
$100{,}000$ at $20$ calls, a single pinned-governor sweep at $n{=}20$
interleaved invocations per size (Section~\ref{sec:controls}). Every
row's \code{direct} and \code{rpc} canonical digests were byte-identical
($\checkmark$). \code{direct} fits $342$\,ns/element ($R^2{>}.9999$); \code{rpc}
fits a $\approx$$17$\,ms fixed cost plus a $911$\,ns/element serialization slope
(95\% CI $889$--$1002$, $R^2{=}.997$), the slope directly observed to overtake the
fixed cost at $N\approx19{,}000$. Bootstrap CIs are in the released
\code{payload\_noturbo\_n20.json}.}
\label{tab:payload}
\centering
\small
\begin{tabular}{@{}rrrrc@{}}
...
\midrule
1      & $0.76$   & $16.5$ & $21{,}708\times$ & $\checkmark$ \\
10     & $4.01$   & $16.1$ & $4{,}025\times$  & $\checkmark$ \\
50     & $17.9$   & $17.2$ & $961\times$      & $\checkmark$ \\
100    & $34.6$   & $16.8$ & $485\times$      & $\checkmark$ \\
250    & $86.1$   & $16.4$ & $190\times$      & $\checkmark$ \\
500    & $172$    & $16.3$ & $95\times$       & $\checkmark$ \\
1000   & $345$    & $17.3$ & $50\times$       & $\checkmark$ \\
2000   & $691$    & $17.0$ & $25\times$       & $\checkmark$ \\
5000   & $1{,}708$ & $21.1$ & $12\times$      & $\checkmark$ \\
10000  & $3{,}410$ & $26.4$ & $8\times$       & $\checkmark$ \\
20000  & $6{,}812$ & $36.5$ & $5\times$       & $\checkmark$ \\
30000  & $10{,}171$ & $47.7$ & $5\times$      & $\checkmark$ \\
40000  & $13{,}590$ & $55.5$ & $4\times$      & $\checkmark$ \\
50000  & $17{,}057$ & $64.6$ & $4\times$      & $\checkmark$ \\
75000  & $25{,}539$ & $85.1$ & $3\times$      & $\checkmark$ \\
100000 & $34{,}282$ & $105.0$ & $3\times$     & $\checkmark$ \\
\bottomrule
\end{tabular}
\end{table}
...
\addplot[mark=*,red!70!black] coordinates {
  (1,16476266)(10,16136632)(50,17192893)(100,16776468)(250,16364385)(500,16318333)(1000,17291125)(2000,16963593)(5000,21103224)(10000,26410197)(20000,36492200)(30000,47651657)(40000,55483578)(50000,64567664)(75000,85056282)(100000,105028790)};
\addplot[mark=square*,blue!70!black] coordinates {
  (1,759)(10,4009)(50,17893)(100,34558)(250,86099)(500,171496)(1000,344727)(2000,690931)(5000,1708301)(10000,3409932)(20000,6811651)(30000,10170737)(40000,13589606)(50000,17056895)(75000,25538996)(100000,34281541)};
\legend{rpc (crossing),direct (in-process)}
\end{loglogaxis}
\end{tikzpicture}
\caption{Payload-cardinality sweep, $N{=}1$ to $100{,}000$. \code{direct} is a
clean slope-$1$ line ($342$\,ns/element); \code{rpc} is a flat $\approx$$17$\,ms
floor that rises once the $911$\,ns/element serialization term climbs above it,
directly crossing break-even at $N\approx19{,}000$. The two curves converge as
...
slope-dominated ($\approx$$5.4\times$ the fixed cost).}
\label{fig:payload}
\end{figure}
...
(Section~\ref{sec:payload}), separating the loopback RPC's $\approx$$17$\,ms fixed
cost from its $911$\,ns/element serialization slope. That cell is now gated by
bootstrap confidence intervals ($B{=}10^4$ percentile pairs bootstrap): the slope
CI is $889$--$1002$\,ns/element and the crossing is directly observed to cross
break-even at $N\approx19{,}000$ and reach $\approx$$5.4\times$ slope-dominance by
...
(Section~\ref{sec:payload}): the loopback RPC resolves into a $\approx$$17$\,ms
fixed cost plus a $911$\,ns/element serialization slope---$2.7\times$ the
...
RPC into a $\approx$$17$\,ms fixed cost and a $911$\,ns/element serialization
slope, observed to cross break-even at $N\approx19{,}000$. HyperProtoBench~\cite{hyperproto} characterizes serialization at scale;
...
into a fixed crossing intercept ($\approx$$2.0$\,\textmu s for the callback
boundary, $\approx$$3.7$\,\textmu s to enter \code{na}) and a placement slope
($\approx$$42\times$, \code{na} vs.\ \code{sv})---so the $1.17\times$ callback
headline is a small fixed cost that amortizes to $\approx$$1.0\times$ as the callee does real work---%
and a sixteen-point payload sweep to $N{=}100{,}000$ resolves the loopback RPC into a
$\approx$$17$\,ms fixed cost plus a $911$\,ns/element serialization slope
(95\% bootstrap CI $889$--$1002$),
$2.7\times$ its in-process twin and directly observed to overtake the fixed cost
at $N\approx19{,}000$. We add low-nanosecond \code{C}-ABI foreign-call
costs, placement-dependent run-to-run variance spanning $1.5$ orders of magnitude,
and pinned-governor cross-runtime floors of $241\times$ and $374\times$ for the
two loopback RPC cells and a $6.8\times$ WebAssembly floor. The suite's most
