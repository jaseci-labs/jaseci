...
and Section~\ref{sec:payload} sixteen payload cardinalities. The eight-point work
sweep (Section~\ref{sec:sweep}) and the two loopback RPC cells
(Section~\ref{sec:controls}) are captured under the pinned governor with turbo
disabled at $n{=}20$; the single-size bridge table below, the \code{iop\_ffi\_*}
and \code{xop\_wasm\_call} cells, and the payload sweep are the earlier capture
(median of five interleaved invocations for family~1, seven for family~2, five to
nine for the payload sweep) and are annotated where their absolutes are
clock-sensitive---re-capture under the same controls is in progress. Every cell's
variants produced byte-identical canonical digests---the oracle passed for all
ten measured kernels at every size, independent of clock.
...
Table~\ref{tab:xruntime} reports the family-2 results. The two loopback RPC
cells (\code{xop\_svc\_split}, \code{xop\_feed}) are re-captured under the pinned
governor and disabled turbo of Section~\ref{sec:controls}, as medians of three
runs at $n{=}20$ interleaved invocations; their run-to-run ratio CV is
$\le$$4\%$ (the \code{direct} baseline is stable to $0.3\%$, so what jitter
remains lives entirely in the RPC numerator). The \code{wasm32} cell is the
earlier median-of-seven capture and awaits re-capture under the same controls.
The marshalled crossings are two-plus orders of magnitude more expensive than
the direct dispatch they replace, as physics demands: a loopback HTTP+JSON RPC
(\code{xop\_svc\_split}) costs $374\times$ its in-process twin and a
generated-client feed (\code{xop\_feed}) $241\times$ its direct provider call,
while a \code{wasm32} export (\code{xop\_wasm\_call}) is $10\times$ a native
bridge under an asymmetric host loop. The crossing-side spread remains wide
(the RPC medians vary $189$--$309$\,M\,ns run to run), so we read these as
\emph{floors}, not precise ratios---and they remain the least apples-to-apples
comparisons in the suite, since the two sides do not share a clock (each carries
its own \code{timing\_source}). Every cross-runtime cell's digests were
byte-identical across all samples.

\begin{table}[t]
\caption{Family~2 cross-runtime. Direct twin vs.\ marshalled crossing. The two
RPC cells are medians of three pinned-governor runs at $n{=}20$; the
\code{wasm32} cell is the earlier median-of-seven capture (re-capture pending).
Timing sources differ per variant (see text); crossing-side spread is wide, so
ratios are floors.}
\label{tab:xruntime}
\centering
\small
...
two loopback RPC cells (plus a $10\times$ WebAssembly floor awaiting re-capture). The suite's most
