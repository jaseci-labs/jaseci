...
integration, witness semantic preservation and compiler regressions.
...
is a fixed $\approx$$2.0$\,\textmu s intercept whose $1.13\times$ headline
...
boundary floor in its own right.) Three findings stand out.
...
suite's cleanest statement of pure boundary overhead; Section~\ref{sec:sweep}
resolves it into a fixed $\approx$$2.0$\,\textmu s per-call intercept.
...
honest story: the Python$\rightarrow$C boundary is $\approx$$54$\,ns for
pybind11 and $\approx$$25$\,ns for a raw C extension, against $\approx$$3$\,ns
per whole iteration (loop, call, and arithmetic together) for Jac-native FFI---so
Jac's advantage over pybind11-class tooling on \emph{mechanism} cost is roughly
an order of magnitude, not the $100\times$-class ratios \code{ctypes}
($\approx$$367$\,ns isolated) exhibits. Part of even the $25$\,ns raw-extension
floor is \code{PyFloat} boxing and refcounting that any CPython extension must
pay---runtime tax, not binding-generator overhead. Every variant reconstructs the
byte-identical digest \code{sqrt:402000}, so the differential-identity oracle now
spans \emph{toolchains}: pybind11, the C extension, \code{ctypes}, \code{cffi},
and Jac-native are provably computing one value from the same foreign calls. The
one comparand we did not build is PyO3 (no Rust toolchain), which pybind11 and
the raw extension bracket.
...
These RPC ratios are floors \emph{for the shipped deployment path}, not a lower
bound on RPC-boundary cost, and we are explicit about why. We decomposed the
$\approx$$15$\,ms per-call RPC floor by timing the same warm provider three ways
(Table~\ref{tab:rpcfloor}): the \code{sv~import} client path ($\approx$$13$\,ms),
raw HTTP with a new connection per call ($\approx$$6$\,ms), and raw HTTP over a
persistent connection ($\approx$$6$\,ms). Keep-alive versus per-call connect
makes no measurable difference, so loopback TCP setup is \emph{not} the cost.
The floor is instead $\approx$$6$\,ms of \emph{server-side dispatch}---the
provider is a full \code{jac start} application server whose route table spans
\code{/admin/*}, \code{/sso/*}, telemetry and \code{/user/*}, so every
\code{feed\_batch} call pays routing, auth-middleware, request-context and
JSON-serialization overhead---plus $\approx$$7$\,ms of client-side
\code{sv~import} marshalling. A minimal RPC endpoint with a tuned client would be
far cheaper; the $374\times$/$241\times$ figures measure Jac's current hosted
path, not the intrinsic price of an RPC boundary. What the sweep \emph{does}
isolate cleanly is the per-element serialization slope
(Section~\ref{sec:payload}), which is unaffected by this fixed framework floor.
...
\begin{table}[t]
\caption{Decomposing the $\approx$$15$\,ms per-call RPC floor. Same warm
\code{xop\_feed\_payload} provider, three client paths, small $N$, pinned clock.
Keep-alive vs.\ new-connection is within noise, so TCP setup is not the cost; the
floor is server-side application-framework dispatch plus \code{sv~import} client
marshalling, neither intrinsic to an RPC boundary.}
\label{tab:rpcfloor}
\centering
\small
\begin{tabular}{@{}lrl@{}}
...
\midrule
\code{sv~import} client (measured) & $\approx$$13$\,ms & server + client marshalling \\
raw HTTP, new connection/call     & $\approx$$6$\,ms  & server + TCP connect \\
raw HTTP, persistent connection   & $\approx$$6$\,ms  & server dispatch only \\
\bottomrule
\end{tabular}
\end{table}
...
Section~\ref{sec:xtool} takes one step onto that axis, adding a matched pybind11
and raw C-extension point to the \code{sqrt} kernel and explicitly separating
mechanism cost from CPython dispatch; the \emph{full} matrix (PyO3, FastAPI and
generated OpenAPI clients, more kernels, per-system warm-up controls) remains the
follow-on paper. We are careful there not to attribute host-interpreter dispatch
